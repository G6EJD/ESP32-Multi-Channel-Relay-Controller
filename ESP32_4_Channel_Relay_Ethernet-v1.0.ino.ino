...
void setup() {
  ...
  StartNetwork();                            // Start WiFi services
...
//#########################################################################################
void StartNetwork() {
  byte mac[] = { 0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED }; // Get the MAC address from your board
  ...
  server.begin();
  Serial.println("\nServer is Connected at: " + Ethernet.localIP().toString());
}
//#########################################################################################
void SetupSystem() {
