...
const char* ServerName = "ChannelController"; // Connect to the server with http://channelcontroller.local/ e.g. if name = "myserver" use http://myserver.local/
...
#define Channel2_Pin    0              // Define the Relay Control pin
#define Channel3_Pin    0              // Define the Relay Control pin
#define Channel4_Pin    0              // Define the Relay Control pin

#define LEDPIN          5              // Define the LED Control pin
#define ChannelReverse  false          // Set to true for Relay that requires a signal HIGH for ON
...
void CheckTimerEvent() {
  ...
  if (Channel1_Override) {                               // If manual override is requested then turn the Channel on
    Channel1_State = "ON";
    ActuateChannel(ON, Channel1);                        // Switch Channel ON if requested
  }
  if (Channel2_Override) {
    Channel2_State = "ON";
    ActuateChannel(ON, Channel2);                        // Switch Channel ON if requested
  }
  if (Channel3_Override) {
    Channel3_State = "ON";
    ActuateChannel(ON, Channel3);                        // Switch Channel ON if requested
  }
  if (Channel4_Override) {
    Channel4_State = "ON";
    ActuateChannel(ON, Channel4);                        // Switch Channel ON if requested
  }
  for (byte channel = 0; channel < Channels; channel++) {
    for (byte dow = 0; dow < 7; dow++) {                 // Look for any valid timer events, if found turn the heating on
      for (byte p = 0; p < NumOfEvents; p++) {
  ...
  if (Channel1_State == "ON") ActuateChannel(ON, Channel1_Pin); else ActuateChannel(OFF, Channel1_Pin);
  if (Channel2_State == "ON") ActuateChannel(ON, Channel2_Pin); else ActuateChannel(OFF, Channel2_Pin);
  if (Channel3_State == "ON") ActuateChannel(ON, Channel3_Pin); else ActuateChannel(OFF, Channel3_Pin);
  if (Channel4_State == "ON") ActuateChannel(ON, Channel4_Pin); else ActuateChannel(OFF, Channel4_Pin);
}
//#########################################################################################
void ActuateChannel(bool demand, int channel) {
  byte channel_pin = 0;
  if (demand) {
    if (ChannelReverse) digitalWrite(channel_pin, HIGH); else digitalWrite(channel_pin, LOW);
    digitalWrite(LEDPIN, HIGH);
    Serial.println("Channel-" + String(channel) + " ON");
  }
  else
  {
    if (ChannelReverse) digitalWrite(channel_pin, LOW); else digitalWrite(channel_pin, HIGH);
    digitalWrite(LEDPIN, LOW);
    Serial.println("Channel-" + String(channel) + " OFF");
  }
}
//#########################################################################################
void append_HTML_header(bool refreshMode) {
  ...
  webpage += "table.sum tr     {padding:0.2em 0.5em 0.2em 0.5em;font-size:1.0em;border:1px solid blue;}";
  webpage += "table.sum td     {padding:0.2em 0.5em 0.2em 0.5em;font-size:1.0em;border:1px solid blue;}";
